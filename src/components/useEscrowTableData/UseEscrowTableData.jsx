...
const UseEscrowTableData = () => {
  ...
  const handleConfirmDelivery = async (flag, itemIdInput) => {
    const res = await confirmDelivery(library.provider, account, parseInt(itemIdInput), flag);
    setNotiMsg({
      title: "",
      content: res
    });
                  ...
                  <td className="use-escrow-table__body__item">
                    {
                      status === "0" ? (
                        <>
                          ...
                          <Button variant="primary" onClick={() => handleApproveRequest(itemId)}>Approve
                            Request</Button>
                        </>
                      ) : status === "2" && provided ? (
                        <div>
                          <Button variant="primary" onClick={() => handleConfirmDelivery(true, itemId)}>Confirm
                            Delivery</Button>
                          <Button variant="secondary" className="ms-2"
                                  onClick={() => handleConfirmDelivery(false, itemId)}>Dispute</Button>
                        </div>
                      ) : ""
                    }
                  </td>
